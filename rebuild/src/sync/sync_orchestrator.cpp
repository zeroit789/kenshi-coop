...
void SyncOrchestrator::StageProcessSpawns() {
    ...
    if (pending > 0 && m_hasPendingTimer) {
        ...
        // ── Deterministic spawn (fix #4) ──
        // Prefer SpawnWithModTemplate: RootObjectFactory::create(0x583400) builds a
        // FRESH request struct from a mod GameData template (live faction/squad/AI),
        // so it needs NO nearby NPC and NO stale pre-call data, and does not hit the
        // faction+0x250 use-after-free. Fire it quickly (3s). Fall back to the legacy
        // struct-clone SpawnCharacterDirect only when no mod template exists (15s grace
        // for NPC-hijack to fire first).
        bool haveModTemplate = m_spawnManager.GetModTemplateCount() > 0;
        int spawnDelaySec = haveModTemplate ? 3 : 15;
        if (pendingDuration.count() >= spawnDelaySec && m_directSpawnAttempts < 5 &&
            (haveModTemplate || m_spawnManager.HasPreCallData())) {
            ...
            if (m_directSpawnAttempts == 0 || sinceLastAttempt.count() >= 3) {
                ...
                if (m_spawnManager.PopNextSpawn(spawnReq)) {
                    ...
                    spdlog::info("SyncOrch: DETERMINISTIC SPAWN attempt #{} for entity {} owner={} slot={} (modTemplate={})",
                                 m_directSpawnAttempts, spawnReq.netId, spawnReq.owner, modSlot, haveModTemplate);
                    nativeHud.AddSystemMessage("Spawning remote player...");

                    // PRIMARY: safe, deterministic factory create from a real mod GameData
                    // (fresh faction/squad/AI built internally — no nearby NPC, no crash).
                    void* character = nullptr;
                    if (m_spawnManager.GetModTemplate(modSlot) != nullptr) {
                        character = m_spawnManager.SpawnWithModTemplate(modSlot, spawnReq.position);
                    }
                    // FALLBACK: legacy struct-clone direct spawn (needs recent pre-call data).
                    if (!character && m_spawnManager.HasPreCallData()) {
                        character = m_spawnManager.SpawnCharacterDirect(&spawnReq.position, modSlot);
                    }
                    if (character) {
