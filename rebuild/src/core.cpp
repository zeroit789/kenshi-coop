...
// Forward declaration (already inside namespace kmp — no extra nesting)
...
void Core::PollForGameLoad() {
    ...
    // ── Hard deadlock-breaker (global-independent) ──
    // The original gate could deadlock forever: the create-burst detector needs the
    // CharacterCreate hook (which may fail to install — one of ~30 patterns that failed
    // at runtime), and BOTH fallbacks above were gated on playerBaseValid||gameWorldValid
    // (GameWorldSingleton resolved UNRESOLVED at runtime). With every detection path dead,
    // OnGameLoaded() never fired and every S2C_EntitySpawn was dropped. PollForGameLoad
    // runs ~every 2s during the Loading phase, so ~45 polls ≈ 90s — comfortably past any
    // real load. After that long with NO detection by ANY path, force the gate open so the
    // sync loop can never be stuck. Spawns arriving meanwhile are held by the deferred buffer.
    if (!m_gameLoaded.load() && s_pollCount >= 45) {
        spdlog::warn("Core::PollForGameLoad — HARD FALLBACK at poll #{}: forcing OnGameLoaded "
                     "(creates={}, globals valid={}/{}). Breaking the load-gate deadlock.",
                     s_pollCount, loadingCreates, playerBaseValid, gameWorldValid);
        m_nativeHud.LogStep("GAME", "Game assumed loaded (hard fallback, poll #" +
                            std::to_string(s_pollCount) + ")");
        entity_hooks::SetLoadingPassthrough(false);
        OnGameLoaded();
    }
}
...
void Core::OnGameLoaded() {
    ...
    if (m_connected.load()) {
        ...
        m_client.SendReliable(writer.Data(), writer.Size()); // Reliable channel 0 (ordered)
