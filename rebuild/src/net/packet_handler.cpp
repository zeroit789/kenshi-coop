...
public:
    ...
    // ── Deferred packet buffer (the missing half of the load-gate fix) ──
    // Entity spawns / world snapshots that arrive before the game world is ready
    // are queued here as raw bytes and replayed in arrival order once the gate
    // opens — instead of being silently dropped (the old "// TODO: Queue for
    // later spawn"), which is the root cause of remote characters never appearing.
    struct DeferredPacket { std::vector<uint8_t> bytes; int channel; };
    static std::vector<DeferredPacket>& DeferredQueue() {
        static std::vector<DeferredPacket> q;
        return q;
    }
    // Defensive: today the queue is only touched on the network thread (HandlePacket
    // runs inside NetworkClient::Update on Core::NetworkThreadFunc), so there is no
    // race yet. The mutex keeps enqueue/drain correct if a game-thread drain is ever
    // added, at negligible cost (the lock is essentially always uncontended).
    static std::mutex& DeferredMutex() {
        static std::mutex m;
        return m;
    }
    static constexpr size_t kMaxDeferred = 8192; // hard cap so a stuck gate can't OOM

    static void EnqueueDeferred(const uint8_t* data, size_t size, int channel) {
        std::lock_guard<std::mutex> lk(DeferredMutex());
        auto& q = DeferredQueue();
        if (q.size() >= kMaxDeferred) return;
        q.push_back({ std::vector<uint8_t>(data, data + size), channel });
    }

    static void DrainDeferred() {
        // Take ownership of the queue under the lock, then replay OUTSIDE the lock so
        // the re-entrant HandlePacket calls below can't deadlock on the same mutex.
        std::vector<DeferredPacket> pending;
        {
            std::lock_guard<std::mutex> lk(DeferredMutex());
            auto& q = DeferredQueue();
            if (q.empty()) return;
        ...
        spdlog::info("PacketHandler: draining {} deferred packet(s) now that the game is ready",
                     pending.size());
        for (auto& p : pending) {
            HandlePacket(p.bytes.data(), p.bytes.size(), p.channel);
        }
    }
    ...
    static void HandlePacket(const uint8_t* data, size_t size, int channel) {
        ...
        if (!Core::Get().IsGameLoaded()) {
            // Entity spawns and world snapshots are deferred (queued as raw bytes) and
            // replayed once the game world exists; everything else is dropped.
            switch (header.type) {
            case MessageType::S2C_EntitySpawn:
            case MessageType::S2C_WorldSnapshot:
                EnqueueDeferred(data, size, channel);
                spdlog::debug("PacketHandler: deferred 0x{:02X} until game loaded ({} queued)",
                              static_cast<uint8_t>(header.type), DeferredQueue().size());
                return;
        ...
        // Game world is ready — flush anything deferred during loading BEFORE the
        // current packet, so entities materialize in their original arrival order.
...
private:
    ...
    static void HandleAllPlayersReady() {
        ...
        // Replay every spawn/snapshot that was deferred while we were loading.
        // (Also auto-drains on the next packet via HandlePacket, but draining here
        //  makes remote characters appear the instant the lobby barrier opens.)
        DrainDeferred();
    }
};
