...
// REQUIRES: m_mutex is held — this runs inside Update()'s std::lock_guard(m_mutex),
// so the m_players reads/writes here (and in GetReadyPlayerCount/GetConnectedPlayerCount)
// are already serialized. Do NOT call this without that lock.
void GameServer::HandlePlayerReady(ConnectedPlayer& player) {
    ...
    // Check if ALL connected players are now ready. Presence in m_players == connected.
    int connected = GetConnectedPlayerCount();
    int ready = GetReadyPlayerCount();

    if (connected >= 1 && ready == connected) {
        // ALL PLAYERS READY - Broadcast the lobby-start barrier to everyone.
        spdlog::info("GameServer: ALL {} PLAYER(S) READY - Broadcasting AllPlayersReady", connected);
...
int GameServer::GetConnectedPlayerCount() const {
    // Presence in the map is the connection invariant.
