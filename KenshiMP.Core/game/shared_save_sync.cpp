// ES: shared_save_sync.cpp - Implementación del modo "partida compartida": ambos jugadores cargan
//     la misma partida con kenshi-online.mod, cada uno controla el personaje "Player N" que le toca
//     por slot/facción, y cada tick se intercambian posiciones y velocidad de juego con el servidor.
// EN: shared_save_sync.cpp - "Shared save" mode implementation: both players load the same save
//     with kenshi-online.mod, each one controls the "Player N" character matching their slot/faction,
//     and every tick positions and game speed are exchanged with the server.
...
// ═══════════════════════════════════════════════════════════════════════════
//  ES: Ambos jugadores cargan la MISMA partida con kenshi-online.mod, que define las facciones y
//      personajes "Player 1"/"Player 2". El servidor asigna a cada uno un string de facción
//      ("10-kenshi-online" = Player 1, "12-kenshi-online" = Player 2). Los personajes ya existen
//      en la partida: no hace falta spawnearlos con la fábrica, solo buscarlos por nombre.
//  EN:
//  SHARED-SAVE SYNC
//
...
// ES: Estado del módulo (nombres, punteros a AnimationClassHuman y Character de cada personaje, flags).
// EN: Module state (names, AnimationClassHuman and Character pointers of each character, flags).
// ── State ──
...
// EN: s_otherRequired: the binary "other" model only applies with 2 players (slot 0/1). With 3+
//     factions (slot >= 2) there is no name-derivable "other", so "both found" must not wait for it.
static bool s_otherRequired = false;

// ES: Limitador de envío de posición (20 Hz).
// EN:
...
// ES: Reintentos de búsqueda de personajes (y throttle del log).
// EN:
// Discovery retry
static auto s_lastDiscoveryLog = std::chrono::steady_clock::time_point{};
static int s_discoveryAttempts = 0;

// ES: Posición remota protegida por mutex: la escribe el hilo de red y la lee el hilo del juego.
//     La velocidad remota es atómica (-1 = no hay actualización pendiente).
// EN:
...
// EN: Faction string -> character name mapping. The lobby string is like "10-kenshi-online.mod";
//     comparisons are done without the ".mod" suffix, so it is stripped first.
...
// EN: Faction StringId -> 0-based slot table. Source of truth: faction-slots.json (generated by
//     ModGen, read by the server); this is a mirror of its 6 StringIds. It is ONLY a fallback when
//     the packet slot is missing (slot < 0); normally the S2C_FactionAssignment slot is used directly.
...
// EN: Derives the character name from the 0-based slot: the mod names player characters
//     "Player N" (1-based). Works for any faction count. Returns "" outside 0..15.
...
// EN: Resolves the OWN character name: prefer the packet slot; if invalid (old packet / -1)
//     fall back to deriving the slot from the faction StringId.
...
// EN: Resolves the "Other" character name (legacy binary 2-player model). With 3+ factions this
//     model is broken; the right fix is discovering remotes by network entity (EntityRegistry/ownerId).
//     Bridge: slot 0 -> "Player 2", slot 1 -> "Player 1", slot >= 2 -> "" (other sync inactive).
...
// ES: Inicializa el módulo con la facción/slot del lobby: resuelve nombres propio y "other",
//     reinicia punteros y estado remoto y avisa por HUD. Si aún no hay facción, Update() reintenta.
// EN: Initializes the module from the lobby faction/slot: resolves own and "other" names,
//     resets pointers and remote state and notifies via HUD. If there is no faction yet, Update() retries.
void Init() {
    ...
    // EN: Slot-based matching: own derived from the slot; other from the 2-player model (empty for slot >= 2).
    ...
    // EN: DIAG-FACMATCH: dump of the faction matching (which slot/faction arrived and what resolved).
    ...
    // EN: Own is MANDATORY: without it we do not know which character the local player controls, so abort.
    ...
    // EN: Other is OPTIONAL: with 3+ factions there is no binary "other" and that is normal; do not abort.
...
// ES: Reinicia todo el estado del módulo (al desconectar).
// EN: Resets all module state (on disconnect).
void Reset() {
    s_initialized = false;
    s_ownFound = false;
    s_otherFound = false;
...
// ES: Lee la posición física por la cadena escribible: AnimationClassHuman+0xC0 -> CharMovement*,
//     +0x320 -> estructura de posición, +0x20/+0x24/+0x28 = x/y/z. Protegido con SEH.
//     Devuelve false si algún puntero no es válido o la posición es (0,0,0).
// EN:
...
// ES: Escribe la posición física por la misma cadena (mueve de verdad al personaje en las físicas).
// EN:
...
// ES: Escribe también la posición en caché del Character (char+0x48/+0x4C/+0x50) para que las
//     lecturas de solo lectura vean el valor nuevo sin esperar al motor. Protegido con SEH.
// EN: Also writes the Character's cached position (char+0x48/+0x4C/+0x50) so read-only readers
//     see the new value without waiting for the engine. SEH-protected.
...
// ES: Tick principal (hilo del juego). Pasos: inicialización perezosa, 1) buscar/revalidar
//     personajes por nombre vía char_tracker_hooks, 2) enviar la posición propia (20 Hz,
//     C2S_PositionUpdate con entityId 0), 3) escribir la posición recibida en el "other",
//     4) aplicar la velocidad de juego recibida.
// EN: Main tick (game thread). Steps: lazy init, 1) find/revalidate characters by name via
//     char_tracker_hooks, 2) send own position (20 Hz, C2S_PositionUpdate with entityId 0),
//     3) write the received position to the "other", 4) apply the received game speed.
void Update(float deltaTime) {
    ...
    // EN: "other" counts as satisfied when not required (3+ factions / per-player) or already found,
    //     so discovery does not block waiting for a character that does not exist.
    ...
    if (!s_ownFound || !otherSatisfied) {
        ...
        // ES: Buscar el personaje propio por nombre (cada tick de búsqueda, por si cambió de zona).
        // EN:
        ...
        // EN: Only search "other" when the binary model applies (2 players); it is then marked as
        //     remote-controlled so the local AI does not fight the network position.
        ...
        // EN: Periodic status log. FIX #2: it used to spam the HUD every 5 s with "Looking for
        //     characters..."; now it goes only to the file log, throttled to once every 30 s.
        ...
        // ES: OJO: este mensaje sale también cuando no se requiere "other" (3+ facciones).
        // EN: NOTE: this message is also shown when "other" is not required (3+ factions).
        ...
        // ES: Revalidar punteros AnimClass cada ~300 ticks (el motor los recrea al cargar zona).
        //     Si la búsqueda falla se invalidan las cachés para no usar punteros reciclados (UAF).
        // EN:
        ...
        if (++s_revalidateCounter % 300 == 0) { // Every ~5 seconds at 60fps
            ...
            if (tc && tc->animClassPtr) {
                ...
                // EN: Found: update caches only if the animClass changed (zone recreation).
                ...
                // EN: Change 1: own lookup failed (character destroyed/reloaded); invalidate own caches so we do
                //     not read/write a recycled pointer; s_ownFound=false forces a new search next tick.
                s_ownAnimClass = nullptr;
                s_ownCharPtr = nullptr;
                s_ownFound = false;
            }
            // Revalidar "other" solo si el modelo binario aplica (2 jugadores).
            // EN: Revalidate "other" only when the binary model applies. When the pointer changes, the old one
            //     is unmarked first so a recycled address is not left remote-controlled forever (change 3.1);
            //     when the lookup fails, caches are invalidated and the old pointer unmarked (change 1).
            if (s_otherRequired) {
    ...
    // ES: PASO 2: leer la posición propia y enviarla al servidor con el formato C2S_PositionUpdate
    //     existente (U8 count + structs CharacterPosition); el servidor la reenvía a los demás.
    // EN:
    ...
    if (sinceSend.count() >= POS_SEND_INTERVAL_MS && s_ownAnimClass) {
        ...
        if (SEH_ReadAnimClassPosition(s_ownAnimClass, myPos)) {
            ...
            // ES: OJO: comentario heredado dudoso; dice que el servidor lee primero un U32 sourcePlayer, pero
            //     se escribe U8(count) como el cliente canónico (core.cpp). Sin verificar contra el servidor.
            // EN:
    ...
    // ES: PASO 3: escribir la última posición recibida en el personaje del otro jugador (física + caché).
    // EN:
    ...
    // ES: PASO 4: aplicar la velocidad de juego recibida (GameWorld+0x700) si difiere, y consumirla.
    // EN:
...
// ES: Hilo de red: guarda la última posición del otro jugador (bajo mutex).
// EN: Network thread: stores the other player's latest position (under mutex).
...
// ES: Hilo de red: guarda la velocidad de juego recibida (atómico).
// EN: Network thread: stores the received game speed (atomic).
void OnRemoteGameSpeedReceived(float speed) {
    s_remoteGameSpeed.store(speed);
}

// ES: Consultas de estado para HUD/diagnóstico.
// EN: Status queries for HUD/diagnostics.
