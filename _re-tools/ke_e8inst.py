# ES: Busca el "instalador" del campo +0xE8: decodifica .text linealmente y, si una escritura
#     mov [regA+0xE8], reg ocurre poco después (<0x120 bytes) de que regA se cargase desde [X+0x448],
#     la imprime. Solo lectura. Uso: python ke_e8inst.py
# EN: Looks for the +0xE8 field "installer": decodes .text linearly and, if a write
#     mov [regA+0xE8], reg happens shortly (<0x120 bytes) after regA was loaded from [X+0x448],
#     prints it. Read-only. Usage: python ke_e8inst.py
...
# EN: Looks for the INSTALLER pattern: within a short window, mov regA,[regB+0x448] followed by mov [regA+0xE8], regPtr
#     READ-ONLY. Decodes .text linearly and keeps a window of instructions.
import pefile
...
# EN: track: for each register, at which IP it was loaded with [X+0x448]
last448={}   # reg -> ip
for ins in dec:
    rip=ins.ip-IB
    # detecta mov regA, [regB+0x448]
    # EN: detects mov regA, [regB+0x448]
    #     (the "if ... if False else True: pass" below is a no-op leftover)
    if ins.mnemonic_name if False else True:
        pass
    m=fmt.format(ins)
    # caso load +0x448
    # EN: case: load from +0x448
    ...
    # EN: case: write [regA+0xE8], regPtr where regA was recently loaded from +0x448
