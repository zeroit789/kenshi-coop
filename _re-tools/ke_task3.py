# ES: Para cada una de las 106 escrituras a +0xE8, desensambla 0x90 bytes previos, extrae registros
#     origen/destino del mov y busca vtables referenciadas (resueltas por RTTI); marca las que caen
#     en la zona Task/Tasker (0x16BD000-0x16BFFFF). Usa ke_re. Uso: python ke_task3.py
# EN: For each of the 106 +0xE8 writes, disassembles the previous 0x90 bytes, extracts the mov's
#     source/destination registers and looks for referenced vtables (resolved via RTTI); flags those
#     in the Task/Tasker area (0x16BD000-0x16BFFFF). Uses ke_re. Usage: python ke_task3.py
...
# EN: Vtables of the Task/Tasker hierarchy. We know Tasker=0x16BDC68, Melee=0x16BE448.
#     The Task hierarchy is usually contiguous in .rdata. We flag any vtable
#     in the 0x16BD000-0x16BF000 range (Task/Tasker area) as "suspicious".
...
# EN: regex to extract destination and source of mov [reg+0xE8],reg
mov_re = re.compile(r'mov \[(\w+)\+0E8h\],(\w+)')
# lea reg, [rip+...] -> direccion absoluta; ke_re disasm ya resuelve a 0x140...
# EN: lea reg, [rip+...] -> absolute address; ke_re disasm already resolves it to 0x140...
...
# EN: For each write, look 0x90 bytes back and detect references to Task-range vtables
for rva in rvas:
    try:
        # desensamblar contexto previo. start = rva-0x90
        # EN: disassemble the previous context. start = rva-0x90
        start = rva - 0x90
    ...
    # EN: find the line of the target write
    ...
    # EN: find vtables referenced in the block (lea with absolute address into .rdata>0x1600000)
    vtrefs = []
    for ln in lines:
    ...
    # EN: flag whether there is a task-range vtable 0x16BD000-0x16BF000
