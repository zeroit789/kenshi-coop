...
while start>o-0x400:
    ...
    start-=1
start_rva=0x62105F-(o-start)
print(f"Funcion contenedora empieza ~0x{start_rva:X}")
code=DATA[start:start+0x100]
for ins in Decoder(64,code,ip=IB+start_rva):
    rr=ins.ip-IB; off=ins.ip-(IB+start_rva)
    ...
    if ins.op0_kind==OpKind.MEMORY and ins.memory_displacement in (0x68,0x70):
        note+=f"   <<< CharBody+0x{ins.memory_displacement:X}"
