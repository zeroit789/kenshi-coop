# -*- coding: utf-8 -*-
# ES: Desensambla getCurrentState / think (0x5CE020, slot vt+0x1D8 de Character, el "pensar" pesado que
#     decide la acción) sin parar en ret, anotando calls y cadenas. Uso: python re_mat7.py
# EN: Disassembles getCurrentState / think (0x5CE020, Character vtable slot vt+0x1D8, the heavy "think"
#     that decides the action) without stopping at ret, annotating calls and strings. Usage: python re_mat7.py
...
def dis(rva,maxlen,label,stop_ret=True):
    ...
    for ins in Decoder(64,code,ip=IB+rva):
        ...
        print(f"0x{rr:08X}  {raw:<28} {fmt.format(ins)}{note}")
        if ins.mnemonic==Mnemonic.INT3: print("  <--PAD"); break
        if stop_ret and ins.mnemonic==Mnemonic.RET: break

# getCurrentState (think pesado, vt+0x1D8) -> decide accion
# EN: getCurrentState (heavy think, vt+0x1D8) -> decides the action
dis(0x5CE020, 0x200, "getCurrentState / think (vt+0x1D8) 0x5CE020", stop_ret=False)
