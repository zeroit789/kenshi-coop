# -*- coding: utf-8 -*-
# ES: Desensambla el constructor de Character / CharBody::create (0x621460) marcando las escrituras a
#     +0x448, +0x648, +0x650, +0x658, +0x68, +0x70, +0x8 y +0x20. Uso: python re_mat9.py
# EN: Disassembles the Character constructor / CharBody::create (0x621460) flagging writes to +0x448,
#     +0x648, +0x650, +0x658, +0x68, +0x70, +0x8 and +0x20. Usage: python re_mat9.py

# Ctor de Character / CharBody::create 0x621460: que se escribe en char+0x448, +0x648, +0x650, +0x658
# EN: Character ctor / CharBody::create 0x621460: what is written into char+0x448, +0x648, +0x650, +0x658
import struct
...
def dis(rva,maxlen,label,highlight_disp=None,stop_ret=False):
    print(f"\n{'='*92}\n=== {label}  RVA 0x{rva:X} ===\n{'='*92}")
    ...
    for ins in Decoder(64,code,ip=IB+rva):
        ...
        # destacar escrituras a offsets de interes
        # EN: highlight writes to offsets of interest
        if ins.mnemonic==Mnemonic.MOV and ins.op0_kind==OpKind.MEMORY:
            d=ins.memory_displacement
            if d in (0x448,0x648,0x650,0x658,0x68,0x70,0x8,0x20):
                note+=f"   <<< WRITE [+0x{d:X}]"
...
dis(0x621460, 0x300, "CharBody::create / ctor 0x621460", stop_ret=False)
